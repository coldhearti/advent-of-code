...
def solve_part_1(input_path: Optional[Path]) -> Any:
    if input_path is not None:
        all_shared_priorities: List[List[bool]] = []
        with open(input_path, mode="r") as fp:
            ...
            for line in lines:
                ...
                all_shared_priorities.append(
                    find_shared_priorities(
                        compartment_1,
                        compartment_2,
                    )
                )
...
def solve_part_2(input_path: Optional[Path]) -> Any:
    if input_path is not None:
        all_group_shared_priorities: List[List[bool]] = []
        with open(input_path, mode="r") as fp:
            lines = fp.readlines()
            for i in range(0, len(lines), GROUP_SIZE):
                line_group = lines[i : i + GROUP_SIZE]
                line_group = [line.strip("\n") for line in line_group]
                all_group_shared_priorities.append(
                    find_shared_priorities(
                        *line_group,
                    )
                )
